...
int main(void)
{
	file = fopen("/home/pi/Desktop/acconeer_rpi_xc111_v1_7_0/rpi_xc111/tmp/test.txt", "w+");
	...
	if (detector_status != ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
		...
		fprintf(file, "Running distance peak detector in blocking mode failed");
	}
	...
	if (detector_status != ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
		...
		fprintf(file, "Running distance peak detector in blocking mode with estimated threshold failed");
	}
...
acc_detector_distance_peak_status_t distance_peak_detect_with_blocking_calls(acc_detector_distance_peak_configuration_t distance_configuration)
{
	...
	if (handle == NULL) {
		...
		fprintf(file, "acc_service_create failed\n");
	...
	if (detector_status == ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
		...
		while (detection_runs > 0) {
			...
			if (detector_status == ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
				...
				fprintf(file, "reflection data not properly retrieved\n");
			}
		...
		fprintf(file, "acc_service_activate() failed\n");
	}
...
acc_detector_distance_peak_status_t distance_peak_detect_with_blocking_calls_with_estimated_threshold(acc_detector_distance_peak_configuration_t distance_configuration)
{
	...
	if (detector_status != ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS)
	{
		...
		fprintf(file, "Setting threshold estimation mode failed.\n");
		return detector_status;
	}
