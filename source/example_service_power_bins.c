...
static void power_bins_callback(const acc_service_handle_t service_handle, const float *power_bins_data,
...
acc_service_status_t execute_power_bins_with_blocking_calls(acc_service_configuration_t power_bins_configuration)
{
	...
	printf("Free space absolute offset: %u mm\n", (unsigned int)(power_bins_metadata.free_space_absolute_offset * 1000.0f));
	...
	if (service_status == ACC_SERVICE_STATUS_OK)
	{
		...
		while (sweep_count > 0)
		{
			...
			if (service_status == ACC_SERVICE_STATUS_OK)
			{
				...
				{
					printf("%u\t%u", (unsigned int)index, (unsigned int)(power_bins_data[index]));
				}
...
void power_bins_callback(const acc_service_handle_t service_handle, const float *power_bins_data,
                         const acc_service_power_bins_result_info_t *result_info, void *user_reference)
{
	(void)service_handle;
