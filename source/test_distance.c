...
int main(void)
{
    ...
    char filename[1000] = "/home/pi/Documents/rpi_xc111/test_distance_data/";
    char filetail[5] = ".txt";
    char filetime[1500];
    ...
    fprintf(file, "[DISTANCE DATA TEST] Current local time and date: %s\n", current_time);
	...
	{
		printf("\n[ERROR] acc_service_distance_configuration_create() failed");
		fprintf(file, "\n[ERROR] acc_service_distance_configuration_create() failed");
		return EXIT_FAILURE;
	}
	printf("\nA111 Initialized\n");
	...
	if (detector_status != ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
		printf("[ERROR] Running distance peak detector in blocking mode failed");
		fprintf(file, "[ERROR] Running distance peak detector in blocking mode failed");
	}
	...
	if (detector_status != ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
		printf("[ERROR] Running distance peak detector in blocking mode with estimated threshold failed");
		fprintf(file, "[ERROR] Running distance peak detector in blocking mode with estimated threshold failed");
	}
...
acc_detector_distance_peak_status_t distance_peak_detect_with_blocking_calls(acc_detector_distance_peak_configuration_t distance_configuration)
{
	...
	fprintf(file, "Running distance peak detector in blocking mode\n");
	...
	fprintf(file, "Actual start: %u mm\n", (unsigned int)(metadata.actual_start_m * 1000.0f + 0.5f));
	fprintf(file, "Actual length: %u mm\n", (unsigned int)(metadata.actual_length_m * 1000.0f + 0.5f));
	fprintf(file, "Actual end: %u mm\n", (unsigned int)((metadata.actual_start_m + metadata.actual_length_m) * 1000.0f + 0.5f));
	fprintf(file, "\n");
	...
	if (detector_status == ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
		...
		while (detection_runs > 0) {
			...
			if (detector_status == ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS) {
				...
				fprintf(file, "Distance detector: Reflections: %u. Seq. nr: %u. (%u-%u mm): %s\n",
				(unsigned int)reflection_count,
				(unsigned int)result_info.sequence_number,
				(unsigned int)(start_m * 1000.0f + 0.5f),
				(unsigned int)(end_m * 1000.0f + 0.5f),
				...
				printf("[ERROR] Reflection data not properly retrieved\n");
				fprintf(file, "[ERROR] Reflection data not properly retrieved\n");
			}
		...
		printf("[ERROR] acc_service_activate() failed\n");
		fprintf(file, "[ERROR] acc_service_activate() failed\n");
	}
...
acc_detector_distance_peak_status_t distance_peak_detect_with_blocking_calls_with_estimated_threshold(acc_detector_distance_peak_configuration_t distance_configuration)
{
	...
	fprintf(file, "Running distance peak detector in blocking mode with estimated threshold\n");
	...
	if (detector_status != ACC_DETECTOR_DISTANCE_PEAK_STATUS_SUCCESS)
	{
		printf("[ERROR] Setting threshold estimation mode failed.\n");
		fprintf(file, "[ERROR] Setting threshold estimation mode failed.\n");
		return detector_status;
	}

	printf("Estimating threshold...\n");
	fprintf(file, "Estimating threshold...\n");
	...
	fprintf(file, "Estimating threshold done...\n");
...
static void configure_detector(acc_detector_distance_peak_configuration_t distance_configuration)
{
	...
	if (sweep_configuration == NULL) {
		printf("\n[ERROR] Sweep configuration not available");
	}
	else {
