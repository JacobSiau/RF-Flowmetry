...
// static void iq_callback(const acc_service_handle_t service_handle, 
//                         const float complex *iq_data, 
...
int main(void)
{
    ...
    char filename[1000] = "/home/pi/Documents/rpi_xc111/test_iq_data/";
    char filetail[5] = ".txt";
    char filetime[1500];
    ...
    fprintf(file, "[IQ DATA TEST] Current local time and date: %s\n", current_time);
	...
	if (iq_configuration == NULL)
	{
        ...
        fprintf(file, "[ERROR] acc_service_iq_configuration_create() failed\n");
		return EXIT_FAILURE;
	}
    ...
    acc_sweep_configuration_requested_range_set(sweep_configuration, .1, 0.4);
	...
	if (service_status == ACC_SERVICE_STATUS_OK)
	{
		...
		while (sweep_count > 0)
		{
			...
			if (service_status == ACC_SERVICE_STATUS_OK)
			{
                ...
                fprintf(file, "IQ result_info->sequence_number: %u\n", (unsigned int)result_info.sequence_number);
				fprintf(file, "IQ data in polar coordinates (r, phi):\n");
				for (uint_fast16_t index = 0; index < iq_metadata.data_length; index++)
				{
                    ...
                    fprintf(file, "(%" PRIfloat ", %" PRIfloat ")\n", ACC_LOG_FLOAT_TO_INTEGER(cabsf(
														    iq_data[index])),
					       ACC_LOG_FLOAT_TO_INTEGER(cargf(iq_data[index])));
				}
			...
			else
			{
                ...
                fprintf(file, "[ERROR] IQ data not properly retrieved\n");
			}
...
// void iq_callback(const acc_service_handle_t service_handle, const float complex *iq_data, const acc_service_iq_result_info_t *result_info,
//                  void *user_reference)
// {
// 	(void)service_handle;
...
// 			printf("(%" PRIfloat ", %" PRIfloat ")\t", ACC_LOG_FLOAT_TO_INTEGER(crealf(iq_data[index])),
